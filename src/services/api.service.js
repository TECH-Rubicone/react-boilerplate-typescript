...
export const API_PATH = config.serviceUrl + config.apiPath;
...
export const getAccessToken = () => _.get(storage.get(AUTH_STORE), ACCESS_TOKEN, null);
...
export const hasPermission = permissions => ({ name }) => {
  const hasAccess = Boolean(_.find(permissions, { name }));
  // config.DEBUG
  // &&console.info('hasPermission => ( resourceName )'
